...
def train(name):
    ...
    env = Hw2Env(render_mode='offscreen')
        ...
        while not done:
            if np.random.random() < epsilon:
                ...
                with torch.no_grad():
                    ...
                    with torch.no_grad():
                        policy_net.eval()
                        q_values = policy_net(state_tensor.reshape(1, 6))
            ...
            if step_counter % UPDATE_FREQ == 0 and len(memory) >= BATCH_SIZE:
                ...
                states = torch.stack([torch.from_numpy(arr) for arr in batch.state]).float().to(device)
                actions = torch.asarray(batch.action).long().to(device)
                rewards = torch.asarray(batch.reward).float().to(device)
                next_states = torch.stack([torch.from_numpy(arr) for arr in batch.next_state]).float().to(device)
...
def test(model_path="hw2_1.pth", experiment_count=1, prediction_epsilon=0.05):
    ...
    env = Hw2Env(output='high', render_mode='gui')
    policy_net = DQN()
    ...
    plt.title("Accumulated Rewards for HW2_1")
    plt.xlabel("Experiment ids")
    plt.ylabel("Total Reward")
    plt.savefig('evaluation/hw2_1_reward.jpg')
    plt.close()

    plt.bar(experiment_ids, rps)
    plt.title("RPS for HW2_1")
    plt.xlabel("Experiment ids")
    plt.ylabel("RPS")
    plt.savefig('evaluation/hw2_1_rps.jpg')
    plt.close()
